...
class Output(object):
    ...
    def writeNodes(self, nodes, startNodeId):
        ...
        for i in range(0, len(nodes), self.maxNodesPerNodeBlock):
            self.writeNodesChunk(
                nodes[i : i + self.maxNodesPerNodeBlock], startNodeId + i
            )
    ...
    def writeNodesChunk(self, nodes, startNodeId):
        ...
        # type, id=1
        blobHeader.append(
            self.makeVar(vType="S", vId=1, content=writableString("OSMData"))
        )
        # datasize, id=3
        blobHeader.append(self.makeVarIdent(vType="V", vId=3))
    ...
    def makeNodePrimitiveBlock(self, startNodeId, nodes):
        ...
        # primitivegroup, id=2
        nodePrimitiveGroup = self.makeNodePrimitiveGroup(startNodeId, nodes)
    ...
    def makeWayPrimitiveBlock(self, startWayId, ways):
        wayPrimitiveBlock = []
        strings = []
        wayPrimitiveGroup = self.makeWayPrimitiveGroup(startWayId, ways, strings)
